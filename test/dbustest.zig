pub fn main() !void {
    ...
    while (true) {
        try handleEvent(service_stream, client_stream);
        @panic("todo");
    }

    // TODO: how do we register out methods with the bus?

    // TODO:
    // connect a dbus client that contains "echo methods" with various type signatures.
    // connect a second dbus client that calls alls these method variations
...
fn handleEvent(service_stream: std.net.Stream, client_stream: std.net.Stream) !void {
    ...
    if (fds[0].revents & std.posix.POLL.IN != 0) {
        serviced += 1;
        try handleServiceMessage(service_stream);
    }
    if (fds[1].revents & std.posix.POLL.IN != 0) {
        serviced += 1;
        try handleClientMessage(client_stream);
    }
    std.debug.assert(serviced == ready);
    return;
}
fn handleServiceMessage(stream: std.net.Stream) !void {
    ...
    // TODO: Read the method call, check if it's "Echo", send response
    @panic("todo: read the member and ");
}

fn handleClientMessage(stream: std.net.Stream) !void {
    var read_buf: [4096]u8 = undefined;
    var socket_reader = dbus.socketReader(stream, &read_buf);
    const reader = socket_reader.interface();
    ...
    switch (fixed.type) {
        ...
        .method_return => {
            std.log.info("Client received method return (Echo response)", .{});
            @panic("todo: parse/verify echo response");
        },
        .error_reply => {
