...
pub fn main() !u8 {
    ...
    const epoll_fd = std.posix.epoll_create1(std.os.linux.EPOLL.CLOEXEC) catch |err| {
...
fn epollAddHandler(epoll_fd: std.posix.fd_t, fd: std.posix.fd_t, handler: *EpollHandler) !void {
    var event = std.os.linux.epoll_event{
        ...
        .data = std.os.linux.epoll_data{ .ptr = @intFromPtr(handler) },
    };
    try std.posix.epoll_ctl(epoll_fd, std.os.linux.EPOLL.CTL_ADD, fd, &event);
}
    ...
    fn handle(base: *EpollHandler) !void {
        ...
        if (self.partial.items.len > 0) {
            ...
            if (processed == self.partial.items.len) {
                ...
                std.mem.copyForwards(u8, self.partial.items[0..new_len], self.partial.items[processed..]);
