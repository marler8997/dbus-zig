...
fn drain(io_w: *std.Io.Writer, data: []const []const u8, splat: usize) error{WriteFailed}!usize {
    ...
    // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    // std.log.info("drain data.len={} splat={}", .{ data.len, splat });
    // for (data, 0..) |chunk, i| {
    //     std.log.info("  data[{}].len = {}", .{ i, chunk.len });
    // }

    const handle = w.net_stream.handle;
    const buffered = io_w.buffered();
    // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    // std.log.info("buffered.len={}", .{buffered.len});
    ...
    while (true) {
        ...
        // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        // std.log.info("sendmsg iovlen={}", .{msg.iovlen});
        // for (0..msg.iovlen) |i| {
        //     std.log.info("  iov[{}].len = {}", .{ i, msg.iov[i].len });
        // }
        const rc = std.os.linux.sendmsg(handle, &msg, std.os.linux.MSG.NOSIGNAL);
        switch (std.posix.errno(rc)) {
            .SUCCESS => {
                // if (w.control_ref) |ref| {
                //     ref.*
                // }
                // Clear control_ref after successful send - FDs only sent once
                // w.control_ref = .empty;
